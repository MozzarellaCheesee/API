...
use crate::structs::{AppState, UserRegistryInput};
...
pub async fn registry(
    ...
    let hashed_password = match user_data.hashing_password(app_state.argon2.clone()) {
    ...
    match user_data.create_user(&hashed_password, &mut conn).await {
        Ok(_) => { HttpResponse::Ok().json(serde_json::json!({"message": "User registered"})) },
