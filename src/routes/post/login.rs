...
use crate::structs::{AuthOutput, Claims, AuthInput, AppState};
...
pub async fn login(
    ...
    let res = match user_data.verify_password(&found_user.password, app_state.argon2.clone()) {
        Ok(result) => {
            result
        },
        Err(err) => {
    ...
    if res {
        ...
        match refresh_token.save_token(&mut conn, &found_user).await {
            Ok(data) => HttpResponse::Ok().json(AuthOutput::new(&a_token, &r_token, &data.device_id.to_string())),
