...
use crate::schema::email_confirmation::dsl::email_confirmation;
use diesel::{QueryDsl, insert_into, BoolExpressionMethods, ExpressionMethods, dsl::exists, select, result::{Error as DieselError}};
use diesel_async::{AsyncPgConnection, RunQueryDsl};
use uuid::Uuid;
use crate::error::CustomError;
use crate::models::{NewUser, User, NewEmailConfirmation};
...
impl UserRegistryInput {
    ...
    pub async fn create_user(&self, hashed_password: &str, conn: &mut AsyncPgConnection) -> Result<(), CustomError> {
        ...
        let inserted_user: User = insert_into(users)
            .values(&new_user)
            .get_result(conn)
            .await?;
        ...
        insert_into(email_confirmation)
            .values(&new_email_confirmation)
            .execute(conn)
            .await?;

        match send_confirm_email(&inserted_user.email, token, &inserted_user.username).await {
            Ok(_) => (),
            Err(err) => return Err(CustomError::EmailConfirmationSandError(err.to_string()))
        }
